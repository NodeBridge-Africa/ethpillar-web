...
export default function ValidatorClientPage() {
  ...
  const handleUpdate = async () => {
    ...
    try {
      const response = await fetch("/api/ssh/execute", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ command: "sudo ethpillar update_validator" }),
      });

      const result = await response.json();

      if (!response.ok) {
        throw new Error(result.message || "Failed to update validator client");
      }

      toast({
        title: "Update Started",
        description:
          "Validator client update process has begun. This may take some time.",
      });
    } catch (error: any) {
      toast({
        title: "Update Failed",
        description: error.message || "Could not update validator client.",
        variant: "destructive",
      });
    } finally {
  ...
  const handleListKeys = async () => {
    setActionLoading((prev) => ({ ...prev, listKeys: true }));
    try {
      const response = await fetch("/api/ssh/execute", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ command: "sudo ethpillar list_validator_keys" }),
      });

      const result = await response.json();

      if (!response.ok) {
        throw new Error(result.message || "Failed to list validator keys");
      }

      toast({
        title: "Validator Keys",
        description:
          result.output || "Key information retrieved. See logs for details.",
      });
    } catch (error: any) {
      toast({
        title: "Failed to List Keys",
        description: error.message || "Could not list validator keys.",
        variant: "destructive",
      });
    } finally {
      setActionLoading((prev) => ({ ...prev, listKeys: false }));
    }
  };
    ...
    <>
      ...
      <Card className="overflow-hidden border-slate-800 bg-slate-900/50 rounded-lg shadow-lg">
        <CardHeader className="border-b border-slate-800 bg-slate-800/40">
          <CardTitle className="flex items-center gap-2 text-lg font-medium text-slate-100">
            <ShieldCheck className="h-5 w-5 text-green-400" /> Service Control (
            {serviceName})
          </CardTitle>
          <CardDescription className="text-slate-400">
            Manage the validator client service. Status updates are live.
          </CardDescription>
        </CardHeader>
      ...
      <Card className="overflow-hidden border-slate-800 bg-slate-900/50 rounded-lg shadow-lg">
        <CardHeader className="border-b border-slate-800 bg-slate-800/40">
          ...
          <CardDescription className="text-slate-400">
            Manage validator client and keys.
          </CardDescription>
        </CardHeader>
