...
export default function ValidatorClientPage() {
  ...
  const handleListKeys = () => {
    handleApiAction(
      "/api/ssh/execute",
      { command: "sudo ethpillar list_validator_keys" },
      `list validator keys`,
      `${serviceName}-listKeys`
    );
  };
        ...
        <CardContent className="p-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          <Button
            ...
            disabled={
              !canPerformActions ||
              isActionInProgress ||
              actionLoading[`${serviceName}-listKeys`]
            }
          >
            {actionLoading[`${serviceName}-listKeys`] ? (
              <Loader2 className="h-4 w-4 animate-spin mr-2" />
