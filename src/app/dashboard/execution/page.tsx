...
import { Server, Play, StopCircle, RefreshCw, Loader2 } from "lucide-react";
  ...
  const handleUpdate = async () => {
    ...
    try {
      const response = await fetch("/api/ssh/execute", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ command: "sudo ethpillar update_execution" }),
      });

      const result = await response.json();

      if (!response.ok) {
        throw new Error(result.message || "Failed to update execution client");
      }

      toast({
        title: "Update Started",
        description:
          "Execution client update process has begun. This may take some time.",
      });
    } catch (error: any) {
      toast({
        title: "Update Failed",
        description: error.message || "Could not update execution client.",
        variant: "destructive",
      });
    } finally {
  ...
  const handleResync = async () => {
    ...
    try {
      const response = await fetch("/api/ssh/execute", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ command: "sudo ethpillar resync_execution" }),
      });

      const result = await response.json();

      if (!response.ok) {
        throw new Error(result.message || "Failed to resync execution client");
      }

      toast({
        title: "Resync Started",
        description:
          "Execution client resync process has begun. This may take a considerable amount of time.",
      });
    } catch (error: any) {
      toast({
        title: "Resync Failed",
        description: error.message || "Could not resync execution client.",
        variant: "destructive",
      });
    } finally {
    ...
    <>
      ...
      <Card className="overflow-hidden border-slate-800 bg-slate-900/50 rounded-lg shadow-lg">
        <CardHeader className="border-b border-slate-800 bg-slate-800/40">
          <CardTitle className="flex items-center gap-2 text-lg font-medium text-slate-100">
            <Server className="h-5 w-5 text-blue-400" /> Service Control (
            {serviceName})
          </CardTitle>
          <CardDescription className="text-slate-400">
            Manage the execution client service. Status updates are live.
          </CardDescription>
        </CardHeader>
      ...
      <Card className="overflow-hidden border-slate-800 bg-slate-900/50 rounded-lg shadow-lg">
        <CardHeader className="border-b border-slate-800 bg-slate-800/40">
          ...
          <CardDescription className="text-slate-400">
            Perform updates and other maintenance tasks.
          </CardDescription>
        </CardHeader>
        <CardContent className="p-6 grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          >
            ...
            Resync Client Data (Use with Caution!)
          </Button>
        </CardContent>
      </Card>
    </>
