...
import { Database, Play, StopCircle, RefreshCw, Loader2 } from "lucide-react";
...
export default function ConsensusClientPage() {
  ...
  const handleUpdate = async () => {
    ...
    try {
      const response = await fetch("/api/ssh/execute", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ command: "sudo ethpillar update_consensus" }),
      });

      const result = await response.json();

      if (!response.ok) {
        throw new Error(result.message || "Failed to update consensus client");
      }

      toast({
        title: "Update Started",
        description:
          "Consensus client update process has begun. This may take some time.",
      });
    } catch (error: any) {
      toast({
        title: "Update Failed",
        description: error.message || "Could not update consensus client.",
        variant: "destructive",
      });
    } finally {
  ...
  const handleCheckSyncing = async () => {
    setActionLoading((prev) => ({ ...prev, syncing: true }));
    try {
      const response = await fetch("/api/ssh/execute", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      if (!response.ok) {
        throw new Error(
          result.message || "Failed to check consensus sync status"
        );
      }
      ...
      toast({
        ...
        description:
          result.output ||
          "Consensus client sync status checked. See logs for details.",
      });
    } catch (error: any) {
      toast({
        title: "Check Failed",
        description: error.message || "Could not check consensus sync status.",
        variant: "destructive",
      });
    } finally {
      setActionLoading((prev) => ({ ...prev, syncing: false }));
    }
  };
